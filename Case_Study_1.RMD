...
- There are 618 Beer name has Ale in it
- There are 41 Beer name has India Pale Ales in it
...
- Next we tried different random seed number and see if this is due to chance
- By trying another randomn seed model, got accuracy rate to 0.9597, increased Sensitivity to 0.33, and Super great Specificity: 0.99 (99% chance of predicting other types of Ales)
- Then we explored what is the best k value to give the most accuracy, after exploring, it is giving k>30, however, this reduced the Sensitivity (successfully predicting India Pale Ales) to almost minimal.
It leads us to explore what k gives best Sensitivity rate(try to improve our model sensitivity rate)?
...
- By using k=3, we got Accuracy rate=0.92, while Sensitivity is around 0.33(successfully predicting India Pale Ales), Specificity=0.95(Successfully predict other Ales)
- **Conclusion: Even we have a great accuracy rate of predicting with our KNN model using IBU and ABV, this maybe due to the fact that the other Ales groups have majority of the data set, and thus it has high chance to categorize an ale to other Ale based on IBU andadjusted ABV alone.**
